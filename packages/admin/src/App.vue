...
onMounted(() => {
  ...
  Promise.all([
    ...
    fetch(`${winConfig.serverPath}/transactions/forms`).then((res) => res.json()).catch(() => []),
    fetch(`${winConfig.serverPath}/entities/forms`).then((res) => res.json()).catch(() => []),
    fetch(`${winConfig.serverPath}/units/forms`).then((res) => res.json()).catch(() => []),
  ])
    ...
    <div v-else>
      ...
      <section class="mb-12">
        ...
        <div class="flex flex-col gap-4">
          <div v-for="model in transactions" :key="model.refId" 
            ...
            <div class="card-body p-5">
              <div class="flex justify-between items-start mb-4 border-b border-base-200 pb-3">
                <div>
                  ...
                  <span class="badge badge-sm badge-ghost font-mono text-xs text-base-content/60">{{ model.refId }}</span>
                </div>
                <div class="badge badge-sm font-bold tracking-wide uppercase" :class="getStatusBadgeClass(model.status)">
                  {{ model.status || "ACTIVE" }}
                </div>
              </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-8 text-sm">
                <div>
                  <h4 class="font-bold border-b border-base-300 pb-1 mb-3 text-xs uppercase tracking-wider">
                    Fields ({{ model.fields?.length || 0 }})
                  </h4>
                  <ul class="space-y-2">
                    <li v-for="f in model.fields" :key="f.refId" class="flex items-center justify-between group">
                      <span class="group-hover:text-primary transition-colors">{{ f.name }}</span>
                      <div class="flex items-center gap-2">
                        <span v-if="f.required" class="badge badge-xs badge-error badge-outline font-bold">REQ</span>
                        <span class="font-mono text-xs text-base-content/50 bg-base-200 px-1 rounded">{{ f.type }}</span>
                      </div>
                    </li>
                  </ul>
                </div>
                <div>
                  ...
                  <ul v-if="getFormsForModel(model.refId).length > 0" class="space-y-2">
                    <li v-for="form in getFormsForModel(model.refId)" :key="form.refId" 
                      ...
                      <code class="text-[10px] text-base-content/50 font-mono">{{ form.refId }}</code>
                    </li>
                  </ul>
      ...
      <section class="mb-12">
        ...
        <div class="flex flex-col gap-4">
          <div v-for="model in entities" :key="model.refId" 
               class="card card-sm bg-base-100 border border-base-300 shadow-sm hover:shadow-md transition-all">
            <div class="card-body p-5">
              <div class="flex justify-between items-start mb-4 border-b border-base-200 pb-3">
                <div>
      ...
      <section class="mb-12">
        ...
        <div class="flex flex-col gap-4">
          <div v-for="model in units" :key="model.refId" 
               class="card card-sm bg-base-100 border border-base-300 shadow-sm hover:shadow-md transition-all">
            <div class="card-body p-5">
              <div class="flex justify-between items-start mb-4 border-b border-base-200 pb-3">
                <div>
