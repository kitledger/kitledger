...
var DOMIterator = class DOMIterator {
	...
	getContexts() {
		...
		ctx.forEach((ctx$1) => {
			const isDescendant = filteredCtx.filter((contexts) => {
				return contexts.contains(ctx$1);
			}).length > 0;
			if (filteredCtx.indexOf(ctx$1) === -1 && !isDescendant) filteredCtx.push(ctx$1);
		});
		return filteredCtx;
	}
	/**
	...
	*/
	forEachIframe(ctx, filter, each, end = () => {}) {
		...
		if (!open) checkEnd();
		ifr.forEach((ifr$1) => {
			if (DOMIterator.matches(ifr$1, this.exclude)) checkEnd();
			else this.onIframeReady(ifr$1, (con) => {
				if (filter(ifr$1)) {
					handled++;
					each(con);
				}
	...
	*/
	createDiacriticsRegExp(str) {
		...
		str.split("").forEach((ch) => {
			dct.every((dct$1) => {
				if (dct$1.indexOf(ch) !== -1) {
					if (handled.indexOf(dct$1) > -1) return false;
					str = str.replace(new RegExp(`[${dct$1}]`, `gm${sens}`), `[${dct$1}]`);
					handled.push(dct$1);
				}
				return true;
			});
