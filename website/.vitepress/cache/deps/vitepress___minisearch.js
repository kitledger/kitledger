...
var MiniSearch = class MiniSearch {
	...
	*/
	executeQuery(query, searchOptions = {}) {
		...
		const results = searchTokenize(query).flatMap((term) => searchProcessTerm(term)).filter((term) => !!term).map(termToQuerySpec(options)).map((query$1) => this.executeQuerySpec(query$1, options));
