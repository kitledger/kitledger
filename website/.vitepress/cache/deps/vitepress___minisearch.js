...
var cleanup = (path) => {
	...
	if (node.size === 0) cleanup(path.slice(0, -1));
		...
		const [key$1, value] = node.entries().next().value;
		if (key$1 !== LEAF) merge(path.slice(0, -1), key$1, value);
	}
};
...
var MiniSearch = class MiniSearch {
		...
		for (const field of fields) {
			...
			for (const term of tokens) {
				...
				if (Array.isArray(processedTerm))
					for (const t of processedTerm) this.addTerm(fieldId, shortDocumentId, t);
	...
	addAllAsync(documents, options = {}) {
		...
		const { chunk, promise } = documents.reduce(({ chunk: chunk$1, promise: promise$1 }, document, i) => {
	...
	executeQuery(query, searchOptions = {}) {
		...
		const results = searchTokenize(query)
			.flatMap((term) => searchProcessTerm(term))
			.filter((term) => !!term)
			.map(termToQuerySpec(options))
			.map((query$1) => this.executeQuerySpec(query$1, options));
	...
	executeQuerySpec(query, searchOptions) {
		...
		if (prefixMatches)
			for (const [term, data$1] of prefixMatches) {
...
var calcBM25Score = (termFreq, matchingCount, totalCount, fieldLength, avgFieldLength, bm25params) => {
	...
	return (
		Math.log(1 + (totalCount - matchingCount + 0.5) / (matchingCount + 0.5)) *
