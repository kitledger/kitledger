import {
	...
	xt as inject,
	zt as onUnmounted,
} from "./vue.runtime.esm-bundler-CixPk8f7.js";
...
function computedEager(fn, options) {
	...
	watchEffect(
		...
		},
		{
			...options,
			flush:
				(_options$flush = options === null || options === void 0 ? void 0 : options.flush) !== null &&
				_options$flush !== void 0
					? _options$flush
					: "sync",
		},
	);
...
var injectLocal = (...args) => {
	...
	const key = args[0];
	const instance =
		(_getCurrentInstance = getCurrentInstance()) === null || _getCurrentInstance === void 0
			? void 0
			: _getCurrentInstance.proxy;
...
function createInjectionState(composable, options) {
	const key =
		(options === null || options === void 0 ? void 0 : options.injectionKey) ||
...
function increaseWithUnit(target, delta) {
	...
	const value =
		((_target$match = target.match(/^-?\d+\.?\d*/)) === null || _target$match === void 0
			? void 0
			: _target$match[0]) || "";
 ...
 */
function objectPick(obj, keys$1, omitUndefined = false) {
	return keys$1.reduce((n, k) => {
		if (k in obj) {
 ...
 */
function objectOmit(obj, keys$1, omitUndefined = false) {
	return Object.fromEntries(
		Object.entries(obj).filter(([key, value]) => {
			return (!omitUndefined || value !== void 0) && !keys$1.includes(key);
		}),
	);
}
...
function extendRef(ref$1, extend, { enumerable = false, unwrap = true } = {}) {
	for (const [key, value] of Object.entries(extend)) {
		...
		if (isRef(value) && unwrap)
			Object.defineProperty(ref$1, key, {
				get() {
					return value.value;
				},
				set(v) {
...
function reactify(fn, options) {
	const unrefFn =
		(options === null || options === void 0 ? void 0 : options.computedGetter) === false ? unref : toValue;
...
function reactifyObject(obj, optionsOrKeys = {}) {
	let keys$1 = [];
	let options;
	if (Array.isArray(optionsOrKeys)) keys$1 = optionsOrKeys;
	else {
		options = optionsOrKeys;
		const { includeOwnProperties = true } = optionsOrKeys;
		keys$1.push(...Object.keys(obj));
		if (includeOwnProperties) keys$1.push(...Object.getOwnPropertyNames(obj));
	}
	return Object.fromEntries(
		keys$1.map((key) => {
			const value = obj[key];
...
function reactiveOmit(obj, ...keys$1) {
	...
	return reactiveComputed(() =>
		typeof predicate === "function"
			? Object.fromEntries(Object.entries(toRefs$1(obj)).filter(([k, v]) => !predicate(toValue(v), k)))
 ...
 */
function reactivePick(obj, ...keys$1) {
	const flatKeys = keys$1.flat();
	const predicate = flatKeys[0];
	return reactiveComputed(() =>
		typeof predicate === "function"
...
function refWithControl(initial, options = {}) {
	...
	function set$1(value, triggering = true) {
		...
		const old = source;
		if (
			((_options$onBeforeChan = options.onBeforeChange) === null || _options$onBeforeChan === void 0
				? void 0
				: _options$onBeforeChan.call(options, value, old)) === false
		)
			return;
		source = value;
		(_options$onChanged = options.onChanged) === null ||
			_options$onChanged === void 0 ||
			_options$onChanged.call(options, value, old);
...
function useArrayIncludes(...args) {
	...
	if (isArrayIncludesOptions(comparator)) {
		...
		formIndex =
			(_comparator$fromIndex = comparator.fromIndex) !== null && _comparator$fromIndex !== void 0
				? _comparator$fromIndex
				: 0;
	...
	comparator =
		(_comparator = comparator) !== null && _comparator !== void 0
			? _comparator
			: (element, value$1) => element === toValue(value$1);
	return computed(() =>
		toValue(list)
			.slice(formIndex)
...
function formatDate(date, formatStr, options = {}) {
	...
	const meridiem =
		(_options$customMeridi = options.customMeridiem) !== null && _options$customMeridi !== void 0
			? _options$customMeridi
			: defaultMeridiem;
	const stripTimeZone = (dateString) => {
		var _dateString$split$;
		return (_dateString$split$ = dateString.split(" ")[1]) !== null && _dateString$split$ !== void 0
			? _dateString$split$
			: "";
	};
	const matches = {
	...
	return formatStr.replace(REGEX_FORMAT, (match, $1) => {
		...
		return (_ref =
			$1 !== null && $1 !== void 0
				? $1
				: (_matches$match = matches[match]) === null || _matches$match === void 0
					? void 0
					: _matches$match.call(matches)) !== null && _ref !== void 0
			? _ref
			: match;
	});
}
...
function useLastChanged(source, options = {}) {
	...
	const ms = shallowRef(
		(_options$initialValue = options.initialValue) !== null && _options$initialValue !== void 0
...
function watchArray(source, cb, options) {
	let oldList = (options === null || options === void 0 ? void 0 : options.immediate)
		? []
...
function computedAsync(evaluationCallback, initialState, optionsOrRef) {
	...
	const {
		...
		evaluating = void 0,
		shallow = true,
		onError = (_globalThis$reportErr = globalThis.reportError) !== null && _globalThis$reportErr !== void 0
			? _globalThis$reportErr
			: noop,
	} = options;
	...
	const reuse = defineComponent({
		...
		setup(props, { attrs, slots }) {
			return () => {
				...
				const vnode =
					(_render$value = render.value) === null || _render$value === void 0
						? void 0
						: _render$value.call(render, {
								...(options.props == null ? keysToCamelKebabCase(attrs) : props),
								$slots: slots,
							});
				return inheritAttrs && (vnode === null || vnode === void 0 ? void 0 : vnode.length) === 1
					? vnode[0]
					: vnode;
			};
		},
			...
			instances.value.map((props) => {
				...
				return h(
					...
					(_slots$default = slots.default) === null || _slots$default === void 0
						? void 0
						: _slots$default.call(slots, props),
				);
			});
			...
			return [
				(_firstParamTargets$va =
					(_firstParamTargets$va2 = firstParamTargets.value) === null || _firstParamTargets$va2 === void 0
						? void 0
						: _firstParamTargets$va2.map((e) => unrefElement(e))) !== null &&
				_firstParamTargets$va !== void 0
					? _firstParamTargets$va
...
function onClickOutside(target, handler, options = {}) {
	...
	if (isIOS && !_iOSWorkaround) {
		...
		Array.from(window$1.document.body.children).forEach((el) =>
	...
	const shouldIgnore = (event) => {
		return toValue(ignore).some((target$1) => {
			if (typeof target$1 === "string")
				return Array.from(window$1.document.querySelectorAll(target$1)).some(
			...
			useEventListener(
				...
				(event) => {
					setTimeout(() => {
						...
						const el = unrefElement(target);
						if (
							((_window$document$acti = window$1.document.activeElement) === null ||
							_window$document$acti === void 0
								? void 0
								: _window$document$acti.tagName) === "IFRAME" &&
							!(el === null || el === void 0 ? void 0 : el.contains(window$1.document.activeElement))
						)
							handler(event);
					}, 0);
				},
...
function onElementRemoval(target, callback, options = {}) {
	const {
		window: window$1 = defaultWindow,
		document: document$1 = window$1 === null || window$1 === void 0 ? void 0 : window$1.document,
		flush = "sync",
	} = options;
...
function onLongPress(target, handler, options) {
	...
	function onRelease(ev) {
		...
		clear();
		if (!(options === null || options === void 0 ? void 0 : options.onMouseUp) || !_posStart || !_startTimestamp)
			return;
		if (
			(options === null ||
			options === void 0 ||
			(_options$modifiers = options.modifiers) === null ||
			_options$modifiers === void 0
				? void 0
				: _options$modifiers.self) &&
		...
		if (
			options === null ||
			options === void 0 ||
			(_options$modifiers2 = options.modifiers) === null ||
			_options$modifiers2 === void 0
				? void 0
				: _options$modifiers2.prevent
		)
			ev.preventDefault();
		if (
			options === null ||
			options === void 0 ||
			(_options$modifiers3 = options.modifiers) === null ||
			_options$modifiers3 === void 0
				? void 0
				: _options$modifiers3.stop
		)
			ev.stopPropagation();
	...
	function onDown(ev) {
		...
		if (
			(options === null ||
			options === void 0 ||
			(_options$modifiers4 = options.modifiers) === null ||
			_options$modifiers4 === void 0
				? void 0
				: _options$modifiers4.self) &&
			ev.target !== elementRef.value
		)
			return;
		clear();
		if (
			options === null ||
			options === void 0 ||
			(_options$modifiers5 = options.modifiers) === null ||
			_options$modifiers5 === void 0
				? void 0
				: _options$modifiers5.prevent
		)
			ev.preventDefault();
		if (
			options === null ||
			options === void 0 ||
			(_options$modifiers6 = options.modifiers) === null ||
			_options$modifiers6 === void 0
				? void 0
				: _options$modifiers6.stop
		)
			ev.stopPropagation();
		posStart = {
			x: ev.x,
	...
	function onMove(ev) {
		...
		if (
			(options === null ||
			options === void 0 ||
			(_options$modifiers7 = options.modifiers) === null ||
			_options$modifiers7 === void 0
				? void 0
				: _options$modifiers7.self) &&
			ev.target !== elementRef.value
		)
			return;
		if (!posStart || (options === null || options === void 0 ? void 0 : options.distanceThreshold) === false)
			return;
		if (
			options === null ||
			options === void 0 ||
			(_options$modifiers8 = options.modifiers) === null ||
			_options$modifiers8 === void 0
				? void 0
				: _options$modifiers8.prevent
		)
			ev.preventDefault();
		if (
			options === null ||
			options === void 0 ||
			(_options$modifiers9 = options.modifiers) === null ||
			_options$modifiers9 === void 0
				? void 0
				: _options$modifiers9.stop
		)
			ev.stopPropagation();
		...
		if (
			...
			((_options$distanceThre = options === null || options === void 0 ? void 0 : options.distanceThreshold) !==
				null && _options$distanceThre !== void 0
				? _options$distanceThre
				: DEFAULT_THRESHOLD)
		)
			clear();
	}
	const listenerOptions = {
		capture:
			options === null ||
			options === void 0 ||
			(_options$modifiers10 = options.modifiers) === null ||
			_options$modifiers10 === void 0
				? void 0
				: _options$modifiers10.capture,
		once:
			options === null ||
			options === void 0 ||
			(_options$modifiers11 = options.modifiers) === null ||
			_options$modifiers11 === void 0
				? void 0
				: _options$modifiers11.once,
	};
	const cleanup = [
	...
	const element = customRef((track, trigger) => {
		...
		return {
			get() {
				...
				track();
				return (_instance$proxy$$refs =
					instance === null ||
					instance === void 0 ||
					(_instance$proxy = instance.proxy) === null ||
					_instance$proxy === void 0
						? void 0
						: _instance$proxy.$refs[key]) !== null && _instance$proxy$$refs !== void 0
					? _instance$proxy$$refs
					: initialValue;
			},
			set() {},
		};
...
function useActiveElement(options = {}) {
	...
	const document$1 =
		(_options$document = options.document) !== null && _options$document !== void 0
			? _options$document
		...
		if (deep) {
			...
			while (element === null || element === void 0 ? void 0 : element.shadowRoot)
				element =
					element === null ||
					element === void 0 ||
					(_element$shadowRoot = element.shadowRoot) === null ||
					_element$shadowRoot === void 0
						? void 0
						: _element$shadowRoot.activeElement;
		}
		return element;
	};
...
function useAnimate(target, keyframes, options) {
	...
	const isSupported = /* @__PURE__ */ useSupported(
		() => window$1 && HTMLElement && "animate" in HTMLElement.prototype,
	);
	...
	useEventListener(
		...
		() => {
			var _animate$value5;
			if (commitStyles)
				(_animate$value5 = animate.value) === null ||
					_animate$value5 === void 0 ||
					_animate$value5.commitStyles();
		},
		listenerOptions,
	);
...
function useAsyncQueue(tasks, options) {
	...
	const result = reactive(
		Array.from(Array.from({ length: tasks.length }), () => ({
	...
	tasks.reduce((prev, curr) => {
		return prev
			.then((prevRes) => {
				...
				if (
					((_result$activeIndex$v = result[activeIndex.value]) === null || _result$activeIndex$v === void 0
						? void 0
						: _result$activeIndex$v.state) === promiseState.rejected &&
					interrupt
				) {
					onFinished();
					return;
...
function useBase64(target, options) {
	...
	function execute() {
		...
		promise.value = new Promise((resolve, reject) => {
			try {
				...
				if (_target == null) resolve("");
					...
					const serialized = (
						(options === null || options === void 0 ? void 0 : options.serializer) ||
		...
		promise.value.then((res) => {
			base64.value =
				(options === null || options === void 0 ? void 0 : options.dataUrl) === false
...
function useBattery(options = {}) {
	...
	const events$1 = ["chargingchange", "chargingtimechange", "dischargingtimechange", "levelchange"];
	const isSupported = /* @__PURE__ */ useSupported(
		() => navigator$1 && "getBattery" in navigator$1 && typeof navigator$1.getBattery === "function",
	);
	...
	if (isSupported.value)
		navigator$1.getBattery().then((_battery) => {
			battery = _battery;
...
function useBluetooth(options) {
	...
	async function requestDevice() {
		...
		try {
			device.value = await (navigator$1 === null || navigator$1 === void 0
				? void 0
				: navigator$1.bluetooth.requestDevice({
						acceptAllDevices,
						filters,
						optionalServices,
					}));
	...
	tryOnMounted(() => {
		...
		if (device.value)
			(_device$value$gatt = device.value.gatt) === null ||
				_device$value$gatt === void 0 ||
				_device$value$gatt.connect();
	});
	tryOnScopeDispose(() => {
		var _device$value$gatt2;
		if (device.value)
			(_device$value$gatt2 = device.value.gatt) === null ||
				_device$value$gatt2 === void 0 ||
				_device$value$gatt2.disconnect();
	});
	return {
		isSupported,
...
function useMediaQuery(query, options = {}) {
	...
	const isSupported = /* @__PURE__ */ useSupported(
		() => window$1 && "matchMedia" in window$1 && typeof window$1.matchMedia === "function",
	);
...
function useBreakpoints(breakpoints, options = {}) {
	...
	function current() {
		const points = Object.keys(breakpoints)
			.map((k) => [k, shortcutMethods[k], pxValue(getValue$1(k))])
			.sort((a, b) => a[2] - b[2]);
	...
	return Object.assign(shortcutMethods, {
		...
		between(a, b) {
			return useMediaQuery(
				() => `(min-width: ${getValue$1(a)}) and (max-width: ${getValue$1(b, -0.1)})`,
				options,
			);
		},
		isGreater(k) {
...
function useBrowserLocation(options = {}) {
	...
	for (const [key, ref$1] of objectEntries(refs))
		watch(ref$1, (value) => {
			if (
				!(window$1 === null || window$1 === void 0 ? void 0 : window$1.location) ||
	...
	const buildState = (trigger) => {
		...
		for (const key of WRITABLE_PROPERTIES)
			refs[key].value =
				window$1 === null ||
				window$1 === void 0 ||
				(_window$location = window$1.location) === null ||
				_window$location === void 0
					? void 0
					: _window$location[key];
		return reactive({
			trigger,
			state: state$1,
			length,
	...
	const update = () => {
		...
		state.value =
			(_permissionStatus$val =
				(_permissionStatus$val2 = permissionStatus.value) === null || _permissionStatus$val2 === void 0
					? void 0
					: _permissionStatus$val2.state) !== null && _permissionStatus$val !== void 0
				? _permissionStatus$val
				: "prompt";
	};
...
function useClipboard(options = {}) {
	...
	function legacyRead() {
		...
		return (_document$getSelectio =
			(_document = document) === null ||
			_document === void 0 ||
			(_document$getSelectio2 = _document.getSelection) === null ||
			_document$getSelectio2 === void 0 ||
			(_document$getSelectio2 = _document$getSelectio2.call(_document)) === null ||
			_document$getSelectio2 === void 0
				? void 0
				: _document$getSelectio2.toString()) !== null && _document$getSelectio !== void 0
...
function useClipboardItems(options = {}) {
	...
	function updateContent() {
		if (isSupported.value)
			navigator$1.clipboard.read().then((items) => {
				content.value = items;
			});
	}
...
function useStorage(key, defaults$1, storage, options = {}) {
	...
	if (!storage)
		try {
			storage = getSSRHandler("getDefaultStorage", () =>
				defaultWindow === null || defaultWindow === void 0 ? void 0 : defaultWindow.localStorage,
			)();
		} catch (e) {
			onError(e);
		}
	 ...
	 */
	if (window$1 && listenToStorageChanges)
		if (storage instanceof Storage) useEventListener(window$1, "storage", onStorageEvent, { passive: true });
	...
	function update(event) {
		...
		try {
			...
			if (event === void 0 || (event === null || event === void 0 ? void 0 : event.newValue) !== serializedData)
...
function useColorMode(options = {}) {
	...
	const updateHTMLAttrs = getSSRHandler("updateHTMLAttrs", (selector$1, attribute$1, value) => {
		const el =
			typeof selector$1 === "string"
				? window$1 === null || window$1 === void 0
					? void 0
					: window$1.document.querySelector(selector$1)
				: unrefElement(selector$1);
		if (!el) return;
	...
	function defaultOnChanged(mode) {
		...
		updateHTMLAttrs(
			selector,
			attribute,
			(_modes$mode = modes[mode]) !== null && _modes$mode !== void 0 ? _modes$mode : mode,
		);
	}
...
function useCssVar(prop, target, options = {}) {
	...
	const elRef = computed(() => {
		...
		return (
			...
			(window$1 === null ||
			window$1 === void 0 ||
			(_window$document = window$1.document) === null ||
			_window$document === void 0
				? void 0
	...
	function updateCssVar() {
		...
		if (el && window$1 && key) {
			...
			variable.value =
				((_window$getComputedSt = window$1.getComputedStyle(el).getPropertyValue(key)) === null ||
				_window$getComputedSt === void 0
					? void 0
					: _window$getComputedSt.trim()) ||
				variable.value ||
				initialValue;
		}
	}
...
function useCycleList(list, options) {
	...
	const index = computed({
		get() {
			...
			let index$1 = (options === null || options === void 0 ? void 0 : options.getIndexOf)
				? options.getIndexOf(state.value, targetList)
				: targetList.indexOf(state.value);
			if (index$1 < 0)
				index$1 =
					(_options$fallbackInde =
						options === null || options === void 0 ? void 0 : options.fallbackIndex) !== null &&
					_options$fallbackInde !== void 0
						? _options$fallbackInde
						: 0;
			return index$1;
		},
	...
	function shift(delta = 1) {
		return set$1(index.value + delta);
	}
	function next(n = 1) {
		return shift(n);
	}
	...
	function getInitialValue() {
		...
		return (_toValue = toValue(
			(_options$initialValue = options === null || options === void 0 ? void 0 : options.initialValue) !== null &&
				_options$initialValue !== void 0
				? _options$initialValue
				: toValue(list)[0],
		)) !== null && _toValue !== void 0
			? _toValue
			: void 0;
	}
	watch(listRef, () => set$1(index.value));
	return {
		state,
		index,
		next,
	...
	const mode = useColorMode({
		...
		onChanged: (mode$1, defaultHandler) => {
			...
			if (options.onChanged)
				(_options$onChanged = options.onChanged) === null ||
					_options$onChanged === void 0 ||
					_options$onChanged.call(options, mode$1 === "dark", defaultHandler, mode$1);
...
function useDeviceMotion(options = {}) {
	...
	const requirePermissions = /* @__PURE__ */ useSupported(
		() =>
			isSupported.value &&
	...
	function init() {
		if (window$1)
			useEventListener(
				window$1,
				"devicemotion",
				createFilterWrapper(eventFilter, (event) => {
					var _event$acceleration,
						_event$acceleration2,
						_event$acceleration3,
						_event$accelerationIn,
						_event$accelerationIn2,
						_event$accelerationIn3,
						_event$rotationRate,
						_event$rotationRate2,
						_event$rotationRate3;
					acceleration.value = {
						x:
							((_event$acceleration = event.acceleration) === null || _event$acceleration === void 0
								? void 0
								: _event$acceleration.x) || null,
						y:
							((_event$acceleration2 = event.acceleration) === null || _event$acceleration2 === void 0
								? void 0
								: _event$acceleration2.y) || null,
						z:
							((_event$acceleration3 = event.acceleration) === null || _event$acceleration3 === void 0
								? void 0
								: _event$acceleration3.z) || null,
					};
					accelerationIncludingGravity.value = {
						x:
							((_event$accelerationIn = event.accelerationIncludingGravity) === null ||
							_event$accelerationIn === void 0
								? void 0
								: _event$accelerationIn.x) || null,
						y:
							((_event$accelerationIn2 = event.accelerationIncludingGravity) === null ||
							_event$accelerationIn2 === void 0
								? void 0
								: _event$accelerationIn2.y) || null,
						z:
							((_event$accelerationIn3 = event.accelerationIncludingGravity) === null ||
							_event$accelerationIn3 === void 0
								? void 0
								: _event$accelerationIn3.z) || null,
					};
					rotationRate.value = {
						alpha:
							((_event$rotationRate = event.rotationRate) === null || _event$rotationRate === void 0
								? void 0
								: _event$rotationRate.alpha) || null,
						beta:
							((_event$rotationRate2 = event.rotationRate) === null || _event$rotationRate2 === void 0
								? void 0
								: _event$rotationRate2.beta) || null,
						gamma:
							((_event$rotationRate3 = event.rotationRate) === null || _event$rotationRate3 === void 0
								? void 0
								: _event$rotationRate3.gamma) || null,
					};
...
function useDevicesList(options = {}) {
	...
	const isSupported = /* @__PURE__ */ useSupported(
		() => navigator$1 && navigator$1.mediaDevices && navigator$1.mediaDevices.enumerateDevices,
	);
	const permissionGranted = shallowRef(false);
	let stream;
...
function useDisplayMedia(options = {}) {
	...
	const enabled = shallowRef(
		(_options$enabled = options.enabled) !== null && _options$enabled !== void 0 ? _options$enabled : false,
	);
	...
	const isSupported = /* @__PURE__ */ useSupported(() => {
		...
		return navigator$1 === null ||
			navigator$1 === void 0 ||
			(_navigator$mediaDevic = navigator$1.mediaDevices) === null ||
			_navigator$mediaDevic === void 0
			? void 0
			: _navigator$mediaDevic.getDisplayMedia;
	});
	const constraint = {
		audio,
		video,
	...
	async function _start() {
		...
		(_stream$value = stream.value) === null ||
			_stream$value === void 0 ||
			_stream$value.getTracks().forEach((t) => useEventListener(t, "ended", stop, { passive: true }));
		return stream.value;
	}
	async function _stop() {
		var _stream$value2;
		(_stream$value2 = stream.value) === null ||
			_stream$value2 === void 0 ||
			_stream$value2.getTracks().forEach((t) => t.stop());
		stream.value = void 0;
	}
	function stop() {
		_stop();
...
function useDraggable(target, options = {}) {
	...
	const handleAutoScroll = (container, targetRect, position$1) => {
		...
		if (scrollSettings.direction === "y" || scrollSettings.direction === "both") {
			if (position$1.y < marginY && scrollTop > 0) deltaY = -speedY;
				...
				position$1.y + targetRect.height > clientHeight - marginY &&
	...
	const start = (e) => {
		...
		const containerRect =
			container === null ||
			container === void 0 ||
			(_container$getBoundin = container.getBoundingClientRect) === null ||
			_container$getBoundin === void 0
				? void 0
		...
		const pos = {
			x:
				e.clientX -
				(container
					? targetRect.left - containerRect.left + (autoScroll ? 0 : container.scrollLeft)
					: targetRect.left),
			y:
				e.clientY -
				(container
					? targetRect.top - containerRect.top + (autoScroll ? 0 : container.scrollTop)
					: targetRect.top),
		};
...
function useDropZone(target, options = {}) {
	...
	if (isClient) {
		...
		const multiple =
			(_options$multiple = _options.multiple) !== null && _options$multiple !== void 0 ? _options$multiple : true;
		const preventDefaultForUnhandled =
			(_options$preventDefau = _options.preventDefaultForUnhandled) !== null && _options$preventDefau !== void 0
			...
			const list = Array.from(
				(_event$dataTransfer$f =
					(_event$dataTransfer = event.dataTransfer) === null || _event$dataTransfer === void 0
						? void 0
						: _event$dataTransfer.files) !== null && _event$dataTransfer$f !== void 0
					? _event$dataTransfer$f
					: [],
			);
		...
		const handleDragEvent = (event, eventType) => {
			...
			const dataTransferItemList =
				(_event$dataTransfer2 = event.dataTransfer) === null || _event$dataTransfer2 === void 0
					? void 0
					: _event$dataTransfer2.items;
			isValid =
				(_ref = dataTransferItemList && checkValidity(dataTransferItemList)) !== null && _ref !== void 0
					? _ref
					: false;
			...
			switch (eventType) {
				case "enter":
					...
					(_options$onEnter = _options.onEnter) === null ||
						_options$onEnter === void 0 ||
						_options$onEnter.call(_options, null, event);
					break;
				case "over":
					var _options$onOver;
					(_options$onOver = _options.onOver) === null ||
						_options$onOver === void 0 ||
						_options$onOver.call(_options, null, event);
					break;
				case "leave":
					var _options$onLeave;
					counter -= 1;
					if (counter === 0) isOverDropZone.value = false;
					(_options$onLeave = _options.onLeave) === null ||
						_options$onLeave === void 0 ||
						_options$onLeave.call(_options, null, event);
					break;
				case "drop":
					counter = 0;
					...
					if (isValid) {
						...
						(_options$onDrop = _options.onDrop) === null ||
							_options$onDrop === void 0 ||
							_options$onDrop.call(_options, currentFiles, event);
					}
					break;
			}
		};
...
function useElementByPoint(options) {
	const {
		...
		multiple,
		scheduler = getDefaultScheduler$7(options),
	} = options;
	const isSupported = /* @__PURE__ */ useSupported(() => {
			...
			element.value = toValue(multiple)
				? (_document$elementsFro =
						document$1 === null || document$1 === void 0
							? void 0
							: document$1.elementsFromPoint(toValue(x), toValue(y))) !== null &&
					_document$elementsFro !== void 0
					? _document$elementsFro
					: []
				: (_document$elementFrom =
							document$1 === null || document$1 === void 0
								? void 0
								: document$1.elementFromPoint(toValue(x), toValue(y))) !== null &&
					  _document$elementFrom !== void 0
					? _document$elementFrom
					: null;
		}),
	};
...
function useEventBus(key) {
	...
	function on(listener) {
		...
		scope === null ||
			scope === void 0 ||
			(_scope$cleanups = scope.cleanups) === null ||
			_scope$cleanups === void 0 ||
			_scope$cleanups.push(_off);
		return _off;
	}
	...
	function emit(event, payload) {
		var _events$get;
		(_events$get = events.get(key)) === null ||
			_events$get === void 0 ||
			_events$get.forEach((v) => v(event, payload));
	}
	return {
		on,
		once,
...
function useEventSource(url, events$1 = [], options = {}) {
	...
	const _init = () => {
		...
		es.onmessage = (e) => {
			...
			event.value = null;
			data.value =
				(_serializer$read = serializer.read(e.data)) !== null && _serializer$read !== void 0
					? _serializer$read
					: null;
			lastEventId.value = e.lastEventId;
		};
		for (const event_name of events$1)
			useEventListener(
				es,
				event_name,
				(e) => {
					...
					data.value =
						(_serializer$read2 = serializer.read(e.data)) !== null && _serializer$read2 !== void 0
							? _serializer$read2
							: null;
					lastEventId.value =
						(_e$lastEventId = e.lastEventId) !== null && _e$lastEventId !== void 0 ? _e$lastEventId : null;
				},
				{ passive: true },
			);
	};
...
function useFavicon(newIcon = null, options = {}) {
	...
	const applyIcon = (icon) => {
		const elements =
			document$1 === null || document$1 === void 0
				? void 0
				: document$1.head.querySelectorAll(`link[rel*="${rel}"]`);
...
function useFetch(url, ...args) {
	...
	const {
		fetch = (_defaultWindow$fetch =
			defaultWindow === null || defaultWindow === void 0 ? void 0 : defaultWindow.fetch) !== null &&
		_defaultWindow$fetch !== void 0
			? _defaultWindow$fetch
	...
	const execute = async (throwOnFailed = false) => {
		...
		if (payload) {
			...
			if (config.payloadType)
				headers["Content-Type"] =
					(_payloadMapping$confi = payloadMapping[config.payloadType]) !== null &&
					_payloadMapping$confi !== void 0
						? _payloadMapping$confi
						: config.payloadType;
		...
		return fetch(context.url, {
			...
			headers: {
				...
				...headersToObject(
					(_context$options = context.options) === null || _context$options === void 0
						? void 0
						: _context$options.headers,
				),
			},
		})
		...
		return (payload, payloadType) => {
			if (!isFetching.value) {
				...
				if (isRef(config.payload))
					watch([refetch, toRef(config.payload)], ([refetch$1]) => refetch$1 && execute(), { deep: true });
				return {
					...shell,
					then(onFulfilled, onRejected) {
...
function useFileDialog(options = {}) {
	...
	const inputRef = computed(() => {
		var _unrefElement;
		const input =
			(_unrefElement = unrefElement(options.input)) !== null && _unrefElement !== void 0
				? _unrefElement
				: document$1
					? document$1.createElement("input")
					: void 0;
		if (input) {
			input.type = "file";
...
function useFileSystemAccess(options = {}) {
	...
	const isSupported = /* @__PURE__ */ useSupported(
		() => window$1 && "showSaveFilePicker" in window$1 && "showOpenFilePicker" in window$1,
	);
	...
	const fileName = computed(() => {
		...
		return (_file$value$name =
			(_file$value = file.value) === null || _file$value === void 0 ? void 0 : _file$value.name) !== null &&
			_file$value$name !== void 0
			? _file$value$name
			: "";
	});
	const fileMIME = computed(() => {
		var _file$value$type, _file$value2;
		return (_file$value$type =
			(_file$value2 = file.value) === null || _file$value2 === void 0 ? void 0 : _file$value2.type) !== null &&
			_file$value$type !== void 0
			? _file$value$type
			: "";
	});
	const fileSize = computed(() => {
		var _file$value$size, _file$value3;
		return (_file$value$size =
			(_file$value3 = file.value) === null || _file$value3 === void 0 ? void 0 : _file$value3.size) !== null &&
			_file$value$size !== void 0
			? _file$value$size
			: 0;
	});
	const fileLastModified = computed(() => {
		var _file$value$lastModif, _file$value4;
		return (_file$value$lastModif =
			(_file$value4 = file.value) === null || _file$value4 === void 0 ? void 0 : _file$value4.lastModified) !==
			null && _file$value$lastModif !== void 0
			? _file$value$lastModif
			: 0;
	});
	...
	async function updateFile() {
		...
		file.value = await ((_fileHandle$value = fileHandle.value) === null || _fileHandle$value === void 0
			? void 0
			: _fileHandle$value.getFile());
	}
	async function updateData() {
		...
		if (type === "Text")
			data.value = await ((_file$value5 = file.value) === null || _file$value5 === void 0
				? void 0
				: _file$value5.text());
		else if (type === "ArrayBuffer")
			data.value = await ((_file$value6 = file.value) === null || _file$value6 === void 0
				? void 0
				: _file$value6.arrayBuffer());
...
function useFocus(target, options = {}) {
	...
	useEventListener(
		...
		(event) => {
			...
			if (
				!focusVisible ||
				((_matches = (_ref = event.target).matches) === null || _matches === void 0
					? void 0
					: _matches.call(_ref, ":focus-visible"))
			)
	...
	const focused = computed({
		...
		set(value) {
			...
			if (!value && innerFocused.value)
				(_targetElement$value = targetElement.value) === null ||
					_targetElement$value === void 0 ||
					_targetElement$value.blur();
			else if (value && !innerFocused.value)
				(_targetElement$value2 = targetElement.value) === null ||
					_targetElement$value2 === void 0 ||
					_targetElement$value2.focus({ preventScroll });
		},
	});
	watch(
		targetElement,
		...
		() => {
			...
			return (_focused.value =
				(_targetElement$value$ =
					(_targetElement$value = targetElement.value) === null ||
					_targetElement$value === void 0 ||
					(_targetElement$value$2 = _targetElement$value.matches) === null ||
					_targetElement$value$2 === void 0
						? void 0
						: _targetElement$value$2.call(_targetElement$value, PSEUDO_CLASS_FOCUS_WITHIN)) !== null &&
				_targetElement$value$ !== void 0
					? _targetElement$value$
					: false);
		},
...
function useFps(options) {
	...
	const every =
		(_options$every = options === null || options === void 0 ? void 0 : options.every) !== null &&
		_options$every !== void 0
			? _options$every
			: 10;
...
function useFullscreen(target, options = {}) {
	...
	const targetRef = computed(() => {
		var _unrefElement;
		return (_unrefElement = unrefElement(target)) !== null && _unrefElement !== void 0
			? _unrefElement
			: document$1 === null || document$1 === void 0
				? void 0
	...
	const isCurrentElementFullScreen = () => {
		if (fullscreenElementMethod)
			return (
				(document$1 === null || document$1 === void 0 ? void 0 : document$1[fullscreenElementMethod]) ===
				targetRef.value
			);
		return false;
	};
	const isElementFullScreen = () => {
		if (fullscreenEnabled.value)
			if (document$1 && document$1[fullscreenEnabled.value] != null) return document$1[fullscreenEnabled.value];
			else {
				const target$1 = targetRef.value;
				if ((target$1 === null || target$1 === void 0 ? void 0 : target$1[fullscreenEnabled.value]) != null)
					return Boolean(target$1[fullscreenEnabled.value]);
			}
		return false;
	};
	async function exit() {
		...
		if (exitMethod.value)
			if ((document$1 === null || document$1 === void 0 ? void 0 : document$1[exitMethod.value]) != null)
				await document$1[exitMethod.value]();
			else {
				const target$1 = targetRef.value;
				if ((target$1 === null || target$1 === void 0 ? void 0 : target$1[exitMethod.value]) != null)
					await target$1[exitMethod.value]();
			}
	...
	async function enter() {
		...
		if (
			requestMethod.value &&
			(target$1 === null || target$1 === void 0 ? void 0 : target$1[requestMethod.value]) != null
		) {
...
function useGeolocation(options = {}) {
	...
	function resume() {
		if (isSupported.value)
			watcher = navigator$1.geolocation.watchPosition(updatePosition, (err) => (error.value = err), {
				enableHighAccuracy,
				maximumAge,
				timeout,
			});
...
function useIdle(timeout = oneMinute, options = {}) {
	const {
		...
		events: events$1 = defaultEvents$1,
		window: window$1 = defaultWindow,
...
function useScroll(element, options = {}) {
	const {
		...
		observe: _observe = { mutation: false },
	...
	function scrollTo(_x, _y) {
		...
		if (!_element) return;
		(_ref = _element instanceof Document ? window$1.document.body : _element) === null ||
			_ref === void 0 ||
			_ref.scrollTo({
		...
		const scrollContainer =
			(_element === null ||
			_element === void 0 ||
			(_document = _element.document) === null ||
			_document === void 0
				? void 0
				: _document.documentElement) ||
			(_element === null || _element === void 0 ? void 0 : _element.documentElement) ||
			_element;
	...
	const setArrivedState = (target) => {
		...
		if (!window$1) return;
		const el =
			(target === null || target === void 0 || (_document2 = target.document) === null || _document2 === void 0
				? void 0
	...
	const onScrollHandler = (e) => {
		...
		if (!window$1) return;
		setArrivedState(
			(_documentElement = e.target.documentElement) !== null && _documentElement !== void 0
				? _documentElement
				: e.target,
		);
	...
	if (
		(observe === null || observe === void 0 ? void 0 : observe.mutation) &&
		element != null &&
		element !== window$1 &&
		element !== document
	)
...
function useInfiniteScroll(element, onLoadMore, options = {}) {
	...
	const state = reactive(
		useScroll(element, {
			...
			offset: {
				[direction]:
					(_options$distance = options.distance) !== null && _options$distance !== void 0
						? _options$distance
						: 0,
				...options.offset,
			},
...
function useKeyModifier(modifier, options = {}) {
	...
	if (document$1)
		events$1.forEach((listenerEvent) => {
			useEventListener(
				document$1,
				listenerEvent,
				(evt) => {
...
function useLocalStorage(key, initialValue, options = {}) {
	...
	return useStorage(
		key,
		initialValue,
		window$1 === null || window$1 === void 0 ? void 0 : window$1.localStorage,
		options,
	);
}
...
function useMemoize(resolver, options) {
	...
	const generateKey = (...args) =>
		(options === null || options === void 0 ? void 0 : options.getKey)
			? options.getKey(...args)
			: JSON.stringify(args);
	/**
...
function useMousePressed(options = {}) {
	...
	const onPressed = (srcType) => (event) => {
		...
		(_options$onPressed = options.onPressed) === null ||
			_options$onPressed === void 0 ||
			_options$onPressed.call(options, event);
	};
	const onReleased = (event) => {
		var _options$onReleased;
		pressed.value = false;
		sourceType.value = null;
		(_options$onReleased = options.onReleased) === null ||
			_options$onReleased === void 0 ||
			_options$onReleased.call(options, event);
	};
...
function useNow(options = {}) {
	...
	const now$1 = ref(/* @__PURE__ */ new Date());
	const update = () => (now$1.value = /* @__PURE__ */ new Date());
...
function useScreenOrientation(options = {}) {
	const { window: window$1 = defaultWindow } = options;
	const isSupported = /* @__PURE__ */ useSupported(
		() => window$1 && "screen" in window$1 && "orientation" in window$1.screen,
	);
...
function usePointerLock(target, options = {}) {
	...
	if (isSupported.value) {
		...
		useEventListener(
			...
			() => {
				...
				const currentElement =
					(_pointerLockElement = document$1.pointerLockElement) !== null && _pointerLockElement !== void 0
						? _pointerLockElement
						: element.value;
		...
		useEventListener(
			...
			() => {
				...
				const currentElement =
					(_pointerLockElement2 = document$1.pointerLockElement) !== null && _pointerLockElement2 !== void 0
						? _pointerLockElement2
						: element.value;
				if (targetElement && currentElement === targetElement) {
	...
	async function lock(e) {
		...
		targetElement =
			e instanceof Event
				? (_unrefElement = unrefElement(target)) !== null && _unrefElement !== void 0
					? _unrefElement
...
function usePointerSwipe(target, options = {}) {
	...
	const eventIsAllowed = (e) => {
		...
		return (_ref =
			(_options$pointerTypes =
				(_options$pointerTypes2 = options.pointerTypes) === null || _options$pointerTypes2 === void 0
					? void 0
					: _options$pointerTypes2.includes(e.pointerType)) !== null && _options$pointerTypes !== void 0
				? _options$pointerTypes
				: isReleasingButton || isPrimaryButton) !== null && _ref !== void 0
			? _ref
			: true;
	};
	const listenerOptions = { passive: true };
	...
	tryOnMounted(() => {
		...
		(_targetRef$value = targetRef.value) === null ||
			_targetRef$value === void 0 ||
			(_targetRef$value = _targetRef$value.style) === null ||
			_targetRef$value === void 0 ||
			_targetRef$value.setProperty("touch-action", "pan-y");
		if (disableTextSelect) {
			var _targetRef$value2, _targetRef$value3, _targetRef$value4;
			(_targetRef$value2 = targetRef.value) === null ||
				_targetRef$value2 === void 0 ||
				(_targetRef$value2 = _targetRef$value2.style) === null ||
				_targetRef$value2 === void 0 ||
				_targetRef$value2.setProperty("-webkit-user-select", "none");
			(_targetRef$value3 = targetRef.value) === null ||
				_targetRef$value3 === void 0 ||
				(_targetRef$value3 = _targetRef$value3.style) === null ||
				_targetRef$value3 === void 0 ||
				_targetRef$value3.setProperty("-ms-user-select", "none");
			(_targetRef$value4 = targetRef.value) === null ||
				_targetRef$value4 === void 0 ||
				(_targetRef$value4 = _targetRef$value4.style) === null ||
				_targetRef$value4 === void 0 ||
				_targetRef$value4.setProperty("user-select", "none");
		}
	});
		...
		new Promise((resolve, reject) => {
			...
			if (!el) {
				...
				Object.entries(attrs).forEach(([name, value]) =>
					el === null || el === void 0 ? void 0 : el.setAttribute(name, value),
				);
				shouldAppend = true;
...
function useSessionStorage(key, initialValue, options = {}) {
	...
	return useStorage(
		key,
		initialValue,
		window$1 === null || window$1 === void 0 ? void 0 : window$1.sessionStorage,
		options,
	);
}
/**
...
function useSorted(...args) {
	...
	if (args.length === 2)
		if (typeof args[1] === "object") {
			...
			options = args[1];
			compareFn =
				(_options$compareFn = options.compareFn) !== null && _options$compareFn !== void 0
					? _options$compareFn
					: defaultCompare;
		} else {
			var _args$;
...
function useStepper(steps, initialStep) {
	...
	const index = ref(
		stepNames.value.indexOf(initialStep !== null && initialStep !== void 0 ? initialStep : stepNames.value[0]),
	);
...
function useStorageAsync(key, initialValue, storage, options = {}) {
	...
	if (window$1 && listenToStorageChanges)
		useEventListener(window$1, "storage", (e) => Promise.resolve().then(() => read(e)), { passive: true });
	if (storage)
		watchWithFilter(
			data,
			async () => {
...
function useTextDirection(options = {}) {
	...
	function getValue$1() {
		...
		return (_ref =
			document$1 === null ||
			document$1 === void 0 ||
			(_document$querySelect = document$1.querySelector(selector)) === null ||
			_document$querySelect === void 0
				? void 0
				: _document$querySelect.getAttribute("dir")) !== null && _ref !== void 0
			? _ref
			: initialValue;
	}
	const dir = ref(getValue$1());
	tryOnMounted(() => (dir.value = getValue$1()));
	if (observe && document$1)
		useMutationObserver(document$1.querySelector(selector), () => (dir.value = getValue$1()), { attributes: true });
	return computed({
		get() {
			return dir.value;
		},
		set(v) {
			...
			if (!document$1) return;
			if (dir.value)
				(_document$querySelect2 = document$1.querySelector(selector)) === null ||
					_document$querySelect2 === void 0 ||
					_document$querySelect2.setAttribute("dir", dir.value);
			else
				(_document$querySelect3 = document$1.querySelector(selector)) === null ||
					_document$querySelect3 === void 0 ||
					_document$querySelect3.removeAttribute("dir");
		},
	});
}
function getRangesFromSelection(selection) {
	...
	const rangeCount =
		(_selection$rangeCount = selection.rangeCount) !== null && _selection$rangeCount !== void 0
...
function useTextSelection(options = {}) {
	...
	const selection = shallowRef(
		(_window$getSelection = window$1 === null || window$1 === void 0 ? void 0 : window$1.getSelection()) !== null &&
	...
	const text = computed(() => {
		...
		return (_selection$value$toSt =
			(_selection$value = selection.value) === null || _selection$value === void 0
				? void 0
				: _selection$value.toString()) !== null && _selection$value$toSt !== void 0
			? _selection$value$toSt
			: "";
	});
...
function useTextareaAutosize(options = {}) {
	...
	const input = toRef(
		(_options$input = options === null || options === void 0 ? void 0 : options.input) !== null &&
			_options$input !== void 0
			? _options$input
			: "",
	);
	const styleProp =
		(_options$styleProp = options === null || options === void 0 ? void 0 : options.styleProp) !== null &&
		_options$styleProp !== void 0
			? _options$styleProp
			: "height";
	...
	function triggerResize() {
		...
		textareaScrollHeight.value =
			(_textarea$value = textarea.value) === null || _textarea$value === void 0
				? void 0
				: _textarea$value.scrollHeight;
	...
	watch(textareaScrollHeight, () => {
		...
		return options === null ||
			options === void 0 ||
			(_options$onResize = options.onResize) === null ||
			_options$onResize === void 0
			? void 0
			: _options$onResize.call(options);
	});
...
function useTimeAgo(time, options = {}) {
	...
	const { now: now$1, ...controls } = useNow({
		scheduler,
		controls: true,
	});
	const timeAgo = computed(() => formatTimeAgo(new Date(toValue(time)), options, toValue(now$1)));
	if (exposeControls)
		return {
			timeAgo,
			...controls,
...
function formatTimeAgo(from, options = {}, now$1 = Date.now()) {
	const {
		max,
	...
	if (typeof max === "string") {
		var _units$find;
		const unitMax =
			(_units$find = units.find((i) => i.name === max)) === null || _units$find === void 0
				? void 0
				: _units$find.max;
...
function useTimeAgoIntl(time, options = {}) {
	...
	const result = computed(() => getTimeAgoIntlResult(new Date(toValue(time)), options, toValue(now$1)));
 ...
 */
function formatTimeAgoIntl(from, options = {}, now$1 = Date.now()) {
	const { parts, resolvedLocale } = getTimeAgoIntlResult(from, options, now$1);
 ...
 */
function getTimeAgoIntlResult(from, options = {}, now$1 = Date.now()) {
	var _options$units;
...
function useTimestamp(options = {}) {
	const {
		...
		offset = 0,
		scheduler = getDefaultScheduler$2(options),
		callback,
	} = options;
...
function useTitle(newTitle = null, options = {}) {
	...
	const originalTitle =
		(_document$title = document$1 === null || document$1 === void 0 ? void 0 : document$1.title) !== null &&
		_document$title !== void 0
			? _document$title
			: "";
	const title = toRef(
		(_ref =
			newTitle !== null && newTitle !== void 0
				...
				: document$1 === null || document$1 === void 0
					? void 0
					: document$1.title) !== null && _ref !== void 0
			? _ref
			: null,
	);
	...
	watch(
		title,
		(newValue, oldValue) => {
			if (newValue !== oldValue && document$1)
				document$1.title = format(newValue !== null && newValue !== void 0 ? newValue : "");
		},
		{ immediate: true },
	);
	if (options.observe && !options.titleTemplate && document$1 && !isReadonly$1) {
		var _document$head;
		useMutationObserver(
			(_document$head = document$1.head) === null || _document$head === void 0
				? void 0
 ...
 */
var TransitionPresets = /* @__PURE__ */ Object.assign(
	{},
	{ linear: identity },
	{
...
function normalizeEasing(easing) {
	...
	return typeof easing === "function"
		? easing
		: (_toValue = toValue(easing)) !== null && _toValue !== void 0
			? _toValue
			: identity;
}
/**
...
function transition(source, from, to, options = {}) {
	...
	const trans =
		typeof options.easing !== "undefined" ? normalizeEasing(options.easing) : normalizeEasing(options.transition);
	...
	return new Promise((resolve) => {
		...
		const tick = () => {
			var _options$abort;
			if (
				(_options$abort = options.abort) === null || _options$abort === void 0
					? void 0
					: _options$abort.call(options)
			) {
				resolve();
				return;
			}
			const now$1 = Date.now();
			source.value = interpolation(fromVal, toVal, ease((now$1 - startedAt) / duration));
			if (now$1 < endAt) window$1 === null || window$1 === void 0 || window$1.requestAnimationFrame(tick);
			else {
				source.value = toVal;
				resolve();
			}
...
function useTransition(source, options = {}) {
	...
	watch(
		...
		async (to) => {
			...
			(_options$onStarted = options.onStarted) === null ||
				_options$onStarted === void 0 ||
				_options$onStarted.call(options);
			await transition(outputRef, outputRef.value, to, {
				...
				abort: () => {
					...
					return (
						id !== currentId ||
						((_options$abort2 = options.abort) === null || _options$abort2 === void 0
							? void 0
							: _options$abort2.call(options))
					);
				},
			});
			(_options$onFinished = options.onFinished) === null ||
				_options$onFinished === void 0 ||
				_options$onFinished.call(options);
		},
		{ deep: true },
	);
	watch(
...
function useUrlSearchParams(mode = "history", options = {}) {
	...
	function constructQuery(params) {
		...
		const index = hash.indexOf("?");
		if (index > 0)
			return `${window$1.location.search || ""}${hash.slice(0, index)}${stringified ? `?${stringified}` : ""}`;
	...
	function write(params, shouldUpdate, shouldWriteHistory = true) {
		...
		if (writeMode === "replace")
			window$1.history.replaceState(
				window$1.history.state,
				window$1.document.title,
				window$1.location.pathname + constructQuery(params),
			);
		else if (shouldWriteHistory)
			window$1.history.pushState(
				window$1.history.state,
				window$1.document.title,
				window$1.location.pathname + constructQuery(params),
			);
...
function useUserMedia(options = {}) {
	...
	const autoSwitch = shallowRef(
		(_options$autoSwitch = options.autoSwitch) !== null && _options$autoSwitch !== void 0
			? _options$autoSwitch
			: true,
	);
	...
	const isSupported = /* @__PURE__ */ useSupported(() => {
		...
		return navigator$1 === null ||
			...
			_navigator$mediaDevic === void 0
			? void 0
			: _navigator$mediaDevic.getUserMedia;
	});
	const stream = shallowRef();
	...
	function _stop() {
		...
		(_stream$value = stream.value) === null ||
			_stream$value === void 0 ||
			_stream$value.getTracks().forEach((t) => t.stop());
		stream.value = void 0;
	}
	function stop() {
		_stop();
...
function useVModel(props, key, emit, options = {}) {
	...
	const _emit =
		...
		(vm === null || vm === void 0 || (_vm$$emit = vm.$emit) === null || _vm$$emit === void 0
			? void 0
			: _vm$$emit.bind(vm)) ||
		(vm === null ||
		vm === void 0 ||
		(_vm$proxy = vm.proxy) === null ||
		_vm$proxy === void 0 ||
		(_vm$proxy = _vm$proxy.$emit) === null ||
		_vm$proxy === void 0
			? void 0
			: _vm$proxy.bind(vm === null || vm === void 0 ? void 0 : vm.proxy));
	let event = eventName;
...
function useVibrate(options) {
	...
	const isSupported = /* @__PURE__ */ useSupported(
		() => typeof navigator$1 !== "undefined" && "vibrate" in navigator$1,
	);
...
function useHorizontalVirtualList(options, list) {
	...
	const calculateRange = createCalculateRange(
		"horizontal",
		overscan,
		createGetOffset(source, itemWidth),
		getViewCapacity,
		resources,
	);
...
function useVerticalVirtualList(options, list) {
	...
	const calculateRange = createCalculateRange(
		"vertical",
		overscan,
		createGetOffset(source, itemHeight),
		getViewCapacity,
		resources,
	);
...
function useWakeLock(options = {}) {
	...
	if (isSupported.value) {
		useEventListener(
			...
			() => {
				...
				requestedType.value =
					(_sentinel$value$type =
						(_sentinel$value = sentinel.value) === null || _sentinel$value === void 0
							? void 0
							: _sentinel$value.type) !== null && _sentinel$value$type !== void 0
						? _sentinel$value$type
						: false;
			},
			{ passive: true },
		...
		whenever(
			() =>
				...
				(document$1 === null || document$1 === void 0 ? void 0 : document$1.visibilityState) === "visible" &&
				requestedType.value,
			(type) => {
	...
	async function forceRequest(type) {
		...
		await ((_sentinel$value2 = sentinel.value) === null || _sentinel$value2 === void 0
			? void 0
			: _sentinel$value2.release());
...
function useWebWorkerFn(fn, options = {}) {
	...
	const callWorker = (...fnArgs) =>
		new Promise((resolve, reject) => {
			...
			(_worker$value = worker.value) === null ||
				_worker$value === void 0 ||
				_worker$value.postMessage([[...fnArgs]]);
...
function useWindowSize(options = {}) {
	...
	if (window$1 && type === "visual" && window$1.visualViewport)
		useEventListener(window$1.visualViewport, "resize", update, listenerOptions);
...
export {
	...
	isDefined as $r,
	useMemoize as $t,
	formatTimeAgoIntl as A,
	syncRefs as Ai,
	useSSRWidth as An,
	createGlobalState as Ar,
	useEventListener as At,
	onLongPress as B,
	tryOnBeforeUnmount as Bi,
	useSpeechSynthesis as Bn,
	extendRef as Br,
	useFullscreen as Bt,
	customStorageEventName as C,
	watchOnce as Ca,
	refDebounced as Ci,
	usePreferredLanguages as Cn,
	computedEager as Cr,
	useDropZone as Ct,
	defaultWindow as D,
	watchWithFilter as Da,
	refWithControl as Di,
	useRafFn as Dn,
	controlledRef as Dr,
	useElementSize as Dt,
	defaultNavigator as E,
	watchTriggerable as Ea,
	refThrottled as Ei,
	usePrevious as En,
	controlledComputed as Er,
	useElementHover as Et,
	onElementRemoval as F,
	toArray as Fi,
	useScrollLock as Fn,
	createSingletonPromise as Fr,
	useFileDialog as Ft,
	transition as G,
	useArrayDifference as Gi,
	useSupported as Gn,
	hyphenate as Gr,
	useInfiniteScroll as Gt,
	provideSSRWidth as H,
	tryOnScopeDispose as Hi,
	useStorage as Hn,
	get as Hr,
	useGeolocation as Ht,
	onKeyDown as I,
	toReactive as Ii,
	useSessionStorage as In,
	debounceFilter as Ir,
	useFileSystemAccess as It,
	useAnimate as J,
	useArrayFind as Ji,
	useTextDirection as Jn,
	increaseWithUnit as Jr,
	useLocalStorage as Jt,
	unrefElement as K,
	useArrayEvery as Ki,
	useSwipe as Kn,
	identity as Kr,
	useIntersectionObserver as Kt,
	onKeyPressed as L,
	toRef as Li,
	useShare as Ln,
	debouncedRef as Lr,
	...
	getSSRHandler as M,
	throttledRef as Mi,
	useScreenSafeArea as Mn,
	createReactiveFn as Mr,
	useEyeDropper as Mt,
	mapGamepadToXbox360Controller as N,
	throttledWatch as Ni,
	useScriptTag as Nn,
	createRef as Nr,
	useFavicon as Nt,
	executeTransition as O,
	whenever as Oa,
	set as Oi,
	useRefHistory as On,
	createEventHook as Or,
	useElementVisibility as Ot,
	onClickOutside as P,
	timestamp as Pi,
	useScroll as Pn,
	createSharedComposable as Pr,
	useFetch as Pt,
	useBattery as Q,
	useArrayJoin as Qi,
	useTimeAgo as Qn,
	...
	toRefs as Ri,
	useSorted as Rn,
	debouncedWatch as Rr,
	useFocusWithin as Rt,
	createUnrefFn as S,
	watchImmediate as Sa,
	refAutoReset as Si,
	usePreferredDark as Sn,
	clamp as Sr,
	useDraggable as St,
	defaultLocation as T,
	watchThrottled as Ta,
	refManualReset as Ti,
	usePreferredReducedTransparency as Tn,
	containsProp as Tr,
	useElementByPoint as Tt,
	setSSRHandler as U,
	tryOnUnmounted as Ui,
	useStorageAsync as Un,
	getLifeCycleTarget as Ur,
	useIdle as Ut,
	onStartTyping as V,
	tryOnMounted as Vi,
	useStepper as Vn,
	...
	useImage as Wt,
	useAsyncState as X,
	useArrayFindLast as Xi,
	useTextareaAutosize as Xn,
	invoke as Xr,
	useManualRefHistory as Xt,
	useAsyncQueue as Y,
	useArrayFindIndex as Yi,
	useTextSelection as Yn,
	injectLocal as Yr,
	useMagicKeys as Yt,
	useBase64 as Z,
	useArrayIncludes as Zi,
	useThrottledRefHistory as Zn,
	isClient as Zr,
	useMediaControls as Zt,
	computedAsync as _,
	watchArray as _a,
	reactify as _i,
	usePointer as _n,
	useWindowSize as _r,
	useDeviceOrientation as _t,
	breakpointsAntDesign as a,
	useDebounce as aa,
	normalizeDate as ai,
	useMutationObserver as an,
	useUserMedia as ar,
	useClipboardItems as at,
	createReusableTemplate as b,
	watchDeep as ba,
	reactiveOmit as bi,
	usePreferredColorScheme as bn,
	bypassFilter as br,
	useDisplayMedia as bt,
	breakpointsMasterCss as c,
	useIntervalFn as ca,
	objectEntries as ci,
	useNow as cn,
	useVibrate as cr,
	useConfirmDialog as ct,
	breakpointsSematic as d,
	useThrottleFn as da,
	pausableFilter as di,
	useOnline as dn,
	useWebNotification as dr,
	useCssVar as dt,
	useArrayReduce as ea,
	isIOS as ei,
	useMemory as en,
	useTimeoutPoll as er,
	useBreakpoints as et,
	breakpointsTailwind as f,
	useTimeout as fa,
	pausableWatch as fi,
	usePageLeave as fn,
	useWebSocket as fr,
	useCurrentElement as ft,
	cloneFnJSON as g,
	useToggle as ga,
	rand as gi,
	usePermission as gn,
	useWindowScroll as gr,
	useDeviceMotion as gt,
	breakpointsVuetifyV3 as h,
	useToString as ha,
	pxValue as hi,
	usePerformanceObserver as hn,
	useWindowFocus as hr,
	useDebouncedRefHistory as ht,
	asyncComputed as i,
	useDateFormat as ia,
	noop as ii,
	useMousePressed as in,
	useUrlSearchParams as ir,
	useClipboard as it,
	formatTimeAgoIntlParts as j,
	throttleFilter as ji,
	useScreenOrientation as jn,
	createInjectionState as jr,
	useEventSource as jt,
	formatTimeAgo as k,
	syncRef as ki,
	useResizeObserver as kn,
	createFilterWrapper as kr,
	useEventBus as kt,
	breakpointsPrimeFlex as l,
	useLastChanged as la,
	objectOmit as li,
	useObjectUrl as ln,
	useVirtualList as lr,
	useCountdown as lt,
	breakpointsVuetifyV2 as m,
	useToNumber as ma,
	provideLocal as mi,
	useParentElement as mn,
	useWebWorkerFn as mr,
	useDark as mt,
	StorageSerializers as n,
	useArrayUnique as na,
	isWorker as ni,
	useMouse as nn,
	useTitle as nr,
	useBrowserLocation as nt,
	breakpointsBootstrapV5 as o,
	useDebounceFn as oa,
	notNullish as oi,
	useNavigatorLanguage as on,
	useVModel as or,
	useCloned as ot,
	breakpointsVuetify as p,
	useTimeoutFn as pa,
	promiseTimeout as pi,
	useParallax as pn,
	useWebWorker as pr,
	useCycleList as pt,
	useActiveElement as q,
	useArrayFilter as qi,
	useTemplateRefsList as qn,
	ignorableWatch as qr,
	useKeyModifier as qt,
	TransitionPresets as r,
	useCounter as ra,
	makeDestructurable as ri,
	useMouseInElement as rn,
	useTransition as rr,
	useCached as rt,
	breakpointsElement as s,
	useInterval as sa,
	now as si,
	useNetwork as sn,
	useVModels as sr,
	useColorMode as st,
	DefaultMagicKeysAliasMap as t,
	useArraySome as ta,
	isObject as ti,
	useMounted as tn,
	useTimestamp as tr,
	useBroadcastChannel as tt,
	breakpointsQuasar as u,
	useThrottle as ua,
	objectPick as ui,
	useOffsetPagination as un,
	useWakeLock as ur,
	useCssSupports as ut,
	computedInject as v,
	watchAtMost as va,
	reactifyObject as vi,
	usePointerLock as vn,
	assert as vr,
	useDevicePixelRatio as vt,
	defaultDocument as w,
	watchPausable as wa,
	refDefault as wi,
	usePreferredReducedMotion as wn,
	computedWithControl as wr,
	useElementBounding as wt,
	createTemplatePromise as x,
	watchIgnorable as xa,
	reactivePick as xi,
	usePreferredContrast as xn,
	camelize as xr,
	useDocumentVisibility as xt,
	createFetch as y,
	watchDebounced as ya,
	reactiveComputed as yi,
	usePointerSwipe as yn,
	autoResetRef as yr,
	useDevicesList as yt,
	onKeyUp as z,
	tryOnBeforeMount as zi,
	useSpeechRecognition as zn,
	eagerComputed as zr,
	useFps as zt,
};
//# sourceMappingURL=dist-DJthFVFr.js.map
