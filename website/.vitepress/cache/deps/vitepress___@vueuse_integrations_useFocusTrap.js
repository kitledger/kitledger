...
var _isInert = function isInert(node, lookUp) {
	...
	return (
		inertAtt === "" ||
		inertAtt === "true" ||
		(lookUp && node && (typeof node.closest === "function" ? node.closest("[inert]") : _isInert(node.parentNode)))
	);
};
/**
...
var _getCandidatesIteratively = function getCandidatesIteratively(elements, includeContainer, options) {
	...
	while (elementsToCheck.length) {
		...
		if (element.tagName === "SLOT") {
			...
			var shadowRoot =
				element.shadowRoot || (typeof options.getShadowRoot === "function" && options.getShadowRoot(element));
			var validShadowRoot =
				!_isInert(shadowRoot, false) && (!options.shadowRootFilter || options.shadowRootFilter(element));
	...
	var getNodeForOption = function getNodeForOption$1(optionName) {
		...
		if (typeof optionValue === "string") {
			try {
				...
				throw new Error(
					"`".concat(optionName, '` appears to be an invalid selector; error="').concat(err.message, '"'),
				);
			}
			if (!node) {
	...
	var getInitialFocusNode = function getInitialFocusNode$1() {
		...
		if (node === void 0 || (node && !isFocusable(node, config.tabbableOptions)))
			if (findContainerIndex(doc.activeElement) >= 0) node = doc.activeElement;
				...
				node =
					(firstTabbableGroup && firstTabbableGroup.firstTabbableNode) || getNodeForOption("fallbackFocus");
			}
